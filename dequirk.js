...
/**
 * @typedef {{sentenceCase: boolean, hoverMode: "none" | "replaced" | "original"}} replacementOptions
 * @typedef {{content: string}} mspaPage
 */

/**
 * @type {{id: string, repl: (options: replacementOptions, page: mspaPage) => void, hidedequirk?: boolean | undefined}[]}
 */
const QUIRKS = [
  {
    id: "aradia",
    repl: logLineReplaceFactory([[/(?<!0_)0(?!_0)/g, "o"]], {colorfilter: ["#A10000"]})
  },
  {
    id: "tavros",
    repl: logLineReplaceFactory([], {colorfilter: ["#0715CD", "#A15000"]}),
    hidedequirk: true
  },
  {
    id: "sollux",
  ...
  {
    id: "karkat",
    repl: logLineReplaceFactory([], {colorfilter: ["#626262"]}),
    hidedequirk: true
  },
  {
    id: "nepeta",
  ...
  {
    id: "kanaya",
    repl: logLineReplaceFactory([], {colorfilter: [""]}),
    hidedequirk: true
  },
  {
    id: "terezi",
  ...
  {
    id: "gamzee",
    repl: logLineReplaceFactory([], {colorfilter: ["#2B0057", "#4200B0"]}),
    hidedequirk: true
  },
  {
    id: "eridan",
    repl: logLineReplaceFactory([[/([wv])\1/g, "$1"]],
...
/**
 * @param {([RegExp, string | ((match: string) => string)])[]} subsitutions
 * @param {{colorfilter?: string[] | undefined, labelfilter?: string[] | undefined, word_repls?: string[][] | undefined}} params
 */
function logLineReplaceFactory(subsitutions, params) {
  /**
   * @param {replacementOptions} opts
   * @param {mspaPage} page
   */
  return function (opts, page) {
    page.content = page.content.replace(
      ...
      (match, color, label, spacer, dialogue) => {
        if (
          (params.colorfilter &&
            !params.colorfilter.includes(color.toUpperCase())) ||
          (params.labelfilter && !params.labelfilter.includes(label))
        ) {
          return match;
        }

        const originalDialogue = dialogue;
        ...
        if (params.word_repls) {
          const dialogueWords = [...dialogue.matchAll(/(?<=\W|^)\w+(?=\W|$)/g)];
          for (let i = dialogueWords.length - 1; i >= 0; i--) {
            const word = dialogueWords[i];
            if (params.word_repls[0].includes(word[0])) {
              dialogue =
                dialogue.slice(0, word.index) +
                params.word_repls[1][params.word_repls[0].indexOf(word[0])] +
                //@ts-ignore
                dialogue.slice(word.index + word[0].length);
            }
          }
        }

        if (opts.sentenceCase) dialogue = capitalize(dialogue);

        switch (opts.hoverMode) {
          case "none":
            return `<span style="color: ${color}">${label}${spacer}${dialogue}<\/span>`;
          case "original":
            return `<span class="dequirk-show" style="color: ${color}">${label}${spacer}${originalDialogue}<\/span>
                    <span class="dequirk-hide" style="color: ${color}">${label}${spacer}${dialogue}<\/span>`;
          case "replaced":
            return `<span class="dequirk-hide" style="color: ${color}">${label}${spacer}${originalDialogue}<\/span>
                    <span class="dequirk-show" style="color: ${color}">${label}${spacer}${dialogue}<\/span>`;
        }
      }
    );
  };
...
.log:hover .dequirk-hide {
    display: inline;
}

.log:hover .dequirk-show {
    display: none;
}`,
    },
  ],
        ...
        options: [
          {
            value: "none",
            label: "None",
            desc: "Don't use mouse hovering",
          },
          {
            value: "original",
            label: "Show Original",
            desc: "Use the original text by default (dequirked text shown on mouse hover)",
          },
          {
            value: "replaced",
            label: "Show Replaced",
            desc: "Use the dequirked text by default (original text shown on mouse hover)",
          },
        ],
      },
      ...QUIRKS.map((quirk) => {
        ...
        if (quirk.hidedequirk)
          options = [
            options[0],
            { value: "dequirk_case", label: "Apply normal letter casing" },
          ];
        return {
          model: quirk.id,
  ...
  computed(api_) {
    ...
    if (api.store.get("hover-mode") == undefined) {
      api.store.set("hover-mode", "none");
    }
  },

  edit(archive) {
    for (const quirk of QUIRKS) {
      if (api.store.get(quirk.id) === "noop") continue;
      // Partially apply the settings we want to use
      /** @type {(page: mspaPage) => void} */
      const replace = quirk.repl.bind(undefined, {
        sentenceCase: api.store.get(quirk.id) === "dequirk_case",
        hoverMode: api.store.get("hover-mode"),
      });
      Object.values(archive.mspa.story).forEach(replace);
    }
  },
};
